...
/**
 * 根据 hash 值过滤任务列表
 * @param {Array} todoList 任务列表数据
 * @param {String} visibility all | completed | active
 * @returns
 */
export function filterTodoList(todoList, visibility) {
  return todoList.filter((item) => {
    if (visibility === 'all') return true;
    else if (visibility === 'completed') return item.completed;
